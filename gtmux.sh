...
action_open() {
  ...
  if [[ -n "$count" ]]; then
    ...
    # no usable ip.txt and no -n, but we have a terminal → guided 3-step setup
    tf err_no_ipfile "$IPS_FILE" >&2
    echo >&2
    local a1 a2 a3
    # ① count or host-file (blank cancels)
    read -rp "$(t setup_p1) " a1
    [[ -n "$a1" ]] || {
      t setup_cancel >&2
      echo >&2
      return 1
    ...
    if [[ "$a1" =~ ^[0-9]+$ ]]; then
      count="$a1"
      # ② label prefix (only meaningful in count mode; blank = none)
      read -rp "$(t setup_p2) " a2
      [[ -n "$a2" ]] && prefix="$a2"
    elif [[ -r "$a1" ]]; then
      IPS_FILE="$a1"
      read_ips || return 1
    else
      tf setup_badfile "$a1" >&2
      echo >&2
      return 1
    fi
...
# device pane id for window number $1 (selection number = device window index)
_dev_pane_of() {
  tmux list-panes -t "$SESSION:$1" -F '#{pane_id}|#{@gtmux}' 2>/dev/null |
    awk -F'|' '$2=="dev"{print $1; exit}'
}

# One monitor tile: poll the LIVE content of a device pane straight from tmux's
# in-memory buffer (no log file, no disk I/O). Repaint only when it changes.
# Cheap when idle: a tile whose monitor window ISN'T on screen does no capture
# at all — so 3 monitors of 10 only spend cycles on the one you're viewing.
action_moncap() {
  local dev="$1" title="$2" self="${TMUX_PANE:-}" last="" cur idle=0 i=0
  trap 'exit 0' INT TERM
  while :; do
    # not the visible monitor → skip all work, just idle
    if [[ "$(tmux display -p -t "$self" '#{window_active}' 2>/dev/null)" != 1 ]]; then
      sleep 2
      continue
    fi
    ((i++ % 15 == 0)) && { sync_lang || true; } # language rarely changes
    cur="$(tmux capture-pane -p -t "$dev" 2>/dev/null)"
    if [[ "$cur" != "$last" ]]; then
      last="$cur"
      idle=0
      printf '\033[H\033[2J%s\n' "$cur"
      tmux select-pane -t "$self" -T "$title  ${T[mon_live]}" 2>/dev/null
    else
      idle=$((idle + 1))
      ((idle % 3 == 0)) && tmux select-pane -t "$self" -T "$title  $(tf mon_stop "$idle")" 2>/dev/null
    fi
    sleep 1
  done
}

# Pick several hosts, tile them in one 'mon-<sel>' window, each tile mirroring a
# host pane live via capture-pane (no log files, no disk I/O — works whether or
# not logging is on). Non-destructive: the real host windows are untouched.
action_monitor() {
  local sel="$*" devf label num dev win title first p
...
bind_keys() {
  ...
  tmux bind-key "$MENU_KEY" display-menu -T "#[align=centre] gtmux " \
    ...
    "$(t m_ctrlc)" "^" "run-shell -b '$SELF _bcastkey C-c'" \
    "$(t m_keys)" k "$_kp_cmd" \
...
# Sticky "GTMUX mode" (Vim-normal-like): after Prefix g, keys work WITHOUT prefix
# until Esc/q. Implemented as a custom key-table whose every binding re-selects
# itself (stay), so it doesn't fall back to root after one key.
bind_mode() {
  local stay="set -t $SESSION key-table gtmux" tbl=gtmux d
  ...
  tmux bind-key -T $tbl "'" \
    "command-prompt -p '#' \"select-window -t $SESSION:%% ; $stay\""
  tmux bind-key -T $tbl Up "previous-window ; $stay"
  tmux bind-key -T $tbl Down "next-window ; $stay"
  tmux bind-key -T $tbl p "previous-window ; $stay"
  tmux bind-key -T $tbl n "next-window ; $stay"
  # broadcast / keys
  tmux bind-key -T $tbl b \
    "command-prompt -p '$(t p_bcast_send)' \"run-shell -b '$SELF _bcast 1 \\\"%%\\\"'\" ; $stay"
  tmux bind-key -T $tbl B \
    "command-prompt -p '$(t p_bcast_nosend)' \"run-shell -b '$SELF _bcast 0 \\\"%%\\\"'\" ; $stay"
  tmux bind-key -T $tbl r "run-shell -b '$SELF _enterall' ; $stay" # run pending broadcast
  tmux bind-key -T $tbl C-c "run-shell -b '$SELF _bcastkey C-c' ; $stay"
  tmux bind-key -T $tbl c "run-shell -b '$SELF _ssh_all' ; $stay"
  # collapse / monitor / log
  tmux bind-key -T $tbl e "run-shell '$SELF _toggle_side \"#{window_id}\"' ; $stay"
  tmux bind-key -T $tbl E "run-shell -b '$SELF _toggle_side_all' ; $stay"
  tmux bind-key -T $tbl m \
    "command-prompt -p '$(t p_monitor)' \"run-shell -b '$SELF _monitor \\\"%%\\\"'\" ; $stay"
  tmux bind-key -T $tbl M \
    "run-shell '$SELF _monitor_close \"#{window_id}\" \"#{window_name}\"' ; $stay"
  tmux bind-key -T $tbl l "run-shell -b '$SELF _log_start' ; $stay"
  tmux bind-key -T $tbl s "run-shell -b '$SELF _log_stop' ; $stay"
  tmux bind-key -T $tbl k "run-shell -b '$SELF _bcastkey C-c' ; $stay" # quick Ctrl-C
  # Catch-all: any other key is swallowed (NOT leaked to the shell) and stays in
  # mode, so you can't accidentally half-type a command. Hint how to leave/type.
  tmux bind-key -T $tbl Any "display-message -- '$(t mode_hint)' ; $stay"
}
